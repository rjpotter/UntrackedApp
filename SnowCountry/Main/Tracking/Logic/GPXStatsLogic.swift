...
func calculateMaxSpeed(locations: [CLLocation], isMetric: Bool) -> Double {
    ...
    if locations.count > 1 {
        for i in 1..<locations.count {
            ...
            let timeInterval = endLocation.timestamp.timeIntervalSince(startLocation.timestamp) // Time in seconds

            if timeInterval > 0 {
                let distanceMeters = endLocation.distance(from: startLocation) // Distance in meters
                let speedMps = distanceMeters / timeInterval // Speed in meters per second (m/s)
                let speedKmh = speedMps * 3.6 // Convert m/s to km/h
                
                let speed = isMetric ? speedKmh : (speedKmh * 0.621371) // Convert km/h to mph if needed
